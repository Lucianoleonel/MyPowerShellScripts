...
if ($null -eq $moduloInstalado) {
    Write-Host -ForegroundColor Cyan "El módulo '$nombreModulo' no está instalado. Se instalará la versión más reciente."
    Install-Module -Name $nombreModulo
} elseif ($moduloInstalado.Version[0] -lt $versionMasReciente) {
    $version = $moduloInstalado.Version[0]
    Write-Host -ForegroundColor Cyan "El módulo '$nombreModulo' está instalado con la versión ($version), pero hay una versión más reciente disponible ($versionMasReciente). Se ejecuta actualización."
